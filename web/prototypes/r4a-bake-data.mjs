 ...
 *
 * 用法：node web/prototypes/r4a-bake-data.mjs [repoRoot]
...
const DATA = {
  ...
  EDGETYPES: {
    enables: { zh: "使能", c: "#7fb069" }, derives: { zh: "衍生", c: "#8fa3bf" },
    applies: { zh: "应用", c: "#b48ead" }, informs: { zh: "反哺", c: "#5e81ac" },
...
const htmlPath = path.join(HERE, "civiliverse-r4a-proto.html");
const html = fs.readFileSync(htmlPath, "utf8");
const out = html.replace(/(\/\*DATA-START\*\/)[\s\S]*?(\/\*DATA-END\*\/)/,
  (_, a, b) => a + "\n" + js + b);
if (out === html) throw new Error("DATA markers not found or unchanged");
