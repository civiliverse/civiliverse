#!/usr/bin/env node
/**
 * Civiliverse · R5a 机检：五档逐档截图＋星云/辉光/取景断言＋交互烟测＋控制台零错误断言
 *
...
const proto = path.resolve(process.argv[2] || path.join(HERE, "..", "civiliverse-r5a-proto.html"));
...
await page.waitForFunction("!!window.__R5A");
const meta = await page.evaluate("window.__R5A.counts");
...
const neb = await page.evaluate("window.__R5A.nebGroups()");
console.log("nebula groups:", neb);
if (neb < 11) throw new Error("nebula groups < 11: " + neb);
/* 图例占宽取景债：开合图例，纵横比须随画幅变化且不产生任何 console error */
const a1 = await page.evaluate("window.__R5A.aspect()");
await page.click("#bLegend"); await settle(300);
const a2 = await page.evaluate("window.__R5A.aspect()");
await page.click("#bLegend"); await settle(300);
console.log("aspect closed/open:", a1, a2);
if (Math.abs(a1 - a2) < 0.005) throw new Error("aspect did not track dock toggle: " + a1 + " vs " + a2);
const clash = await page.evaluate("window.__R5A.clashCount()");
console.log("clash-registered labels:", clash);
...
async function lvl() { return page.evaluate("window.__R5A.level()"); }

/* ── 逐档截图（缩放尺直达） ── */
const expect = [0, 1, 2, 3];
for (let i = 0; i < 4; i++) {
  await page.evaluate(`window.__R5A.zoomToLevel(${i})`);
  await settle();
  const l = await lvl();
...
await page.evaluate("window.__R5A.centerOn('movable-type')");
await settle();
await page.evaluate("window.__R5A.zoomTo(6.2)");
await settle(1100);
...
/* ── 交互烟测 ── */
/* 退出 L4 自动收面板 */
await page.evaluate("window.__R5A.zoomToLevel(2)");
await settle(900);
...
/* 选星→星球层→升空 */
await page.evaluate("window.__R5A.select('printing-revolution')");
await settle(300);
...
await settle(200);
/* #55 星球层：R5 起已脱孤（3 条 informs 前置＋1 条 inspires 出边），atrocity 禁 AI／mourn 样式不变 */
await page.evaluate("window.__R5A.select('holocaust')");
await settle(200);
await page.click("#drawerBody .land");
await settle(400);
...
await page.click("#ascend");
await settle(200);
await page.evaluate("window.__R5A.clearSel()");
/* 回路聚焦开合 */
await page.evaluate("window.__R5A.focusLoop(true)");
await settle(1000);
await shot("shot-loop");
await page.evaluate("window.__R5A.focusLoop(false)");
await settle(700);
/* 区域双击聚焦（宋朝→L3） */
await page.evaluate("window.__R5A.zoomRegion('song-dynasty')");
await settle(900);
if ((await lvl()) !== 3) throw new Error("zoomRegion should land in L3");
await shot("shot-region-song");
/* 背景介绍页 */
await page.evaluate("window.__R5A.zoomToLevel(1)");
await settle(900);
...
console.log("R5A VERIFY PASS: 5 level shots + nebula/glow/aspect asserts + smoke, zero console errors/warnings");
